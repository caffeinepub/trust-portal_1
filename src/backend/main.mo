...
actor {
  ...
  // Any authenticated (non-anonymous) user can upload photos
  public shared ({ caller }) func uploadPhoto(title : Text, image : Storage.ExternalBlob) : async () {
    if (caller.isAnonymous()) {
      Runtime.trap("Unauthorized: Must be logged in to upload photos");
    };
    let newPhoto : Photo = {
      id = Time.now().toText();
      title;
  ...
  // Any authenticated (non-anonymous) user can upload videos
  ...
  public shared ({ caller }) func deletePhoto(id : Text) : async Bool {
    if (caller.isAnonymous()) {
      Runtime.trap("Unauthorized: Must be logged in to delete photos");
    };
